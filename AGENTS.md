...
- 修改范围保持最小，不顺手重构无关代码。
- 生成代码时默认补充适量的“理解型注释”，帮助后续开发者快速理解关键业务意图和非直观逻辑。
- 注释优先解释“为什么这样做、这段逻辑解决什么问题、有哪些边界条件”，不要逐行翻译代码。

### 注释规则

以下位置默认应该有简短注释，除非代码本身已经非常直观：

- 页面或模块中的关键业务流程，例如查询、提交、状态切换、初始化顺序。
- 不容易从变量名直接理解的条件判断、权限判断和边界处理。
- 接口数据与页面数据之间的转换、兼容处理或特殊 payload 组装。
- watch / lifecycle / 路由守卫 / 缓存等存在副作用的逻辑，说明触发原因和目的。
- 为第三方组件、浏览器行为或后端契约做的 workaround，说明为什么不能使用更直接的写法。
- 复杂计算、特殊排序、过滤、映射规则，说明业务规则而不是重复表达语法。

推荐：

```ts
// 切换查询条件后回到第一页，避免当前页在新结果集中不存在。
pagination.pageCurrent = 1
await getTableData()

// API 层已经统一提示通用错误，这里只负责恢复页面 loading 状态。
try {
...
注释密度保持克制：一个清晰函数通常只需要 0～2 条关键注释；不要为了“有注释”给每个变量、赋值、按钮事件和模板节点加注释。
