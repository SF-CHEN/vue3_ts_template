...
export const useSettingsStore = defineStore("settings", () => {
  ...
  for (const [key, value] of Object.entries(layoutsConfig)) {
    ...
    state[key as SettingsStoreKey] = refValue as SettingsStore[SettingsStoreKey]
    // 监听每个响应式变量
    watch(refValue, () => {
      // 缓存
  ...
  const getCacheData = () => {
    ...
    for (const [key, value] of Object.entries(state)) {
      settings[key as SettingsStoreKey] = value.value as LayoutsConfig[SettingsStoreKey]
    }
    return settings
  }
